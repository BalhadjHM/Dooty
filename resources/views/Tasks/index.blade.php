...
@if($userId)
    <x-layouts.app>
        ...
        <x-slot name="content">
            ...
            <!-- Tasks -->
            <x-container class="flex flex-col gap-2">
                ...
                @foreach($tasks as $task)
                    <div id="task-card" data-user-id="{{ $userId }}" data-space-id="{{ $spaceId }}" data-task-id="{{ $task->id }}" class="w-full px-4 py-2 flex flex-row justify-between items-stretch gap-4 divide-x divide-slate-100 border border-slate-200 rounded-md shadow-sm cursor-pointer duration-300 ease-in-out hover:shadow hover:scale-[1.01] {{ $task->status == 0 ? 'line-through' : '' }}">
                        <div class="flex flex-col grow-0 justify-start items-start gap-2">
                            <div class="flex flex-col justify-center items-start">
                                ...
                                <div class="flex flex-row items-center justify-center gap-2">
                                    ...
                                    <p class="text-xs text-gray-600 open-sans-regular">{{ Carbon::parse($task->due_date)->format('D, M') . " " . Carbon::parse($task->due_date)->format('d') }}</p>                                </div>
                            </div>
                        ...
                        <div class="pl-3 flex flex-col gap-2 items-start">
                            ...
                            <x-dropdown>
                                <!-- Edit -->
                                <a href="{{ route('task.edit', ['userId' => $userId, 'spaceId' => $spaceId, 'taskId' => $task->id]) }}" class="flex items-center gap-x-3.5 py-2 px-3 rounded-lg text-sm text-gray-800 hover:bg-accent focus:outline-none focus:bg-gray-200">
                            ...
                            @if($task->status != 2)
                                <x-popover>
                                    <form method="post" action="{{ route('task.star', ['userId' => $userId, 'spaceId' => $spaceId, 'taskId' => $task->id]) }}" id="dropdown" class="m-0">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" id="dropdown" class="flex justify-center items-center size-7 sm:size-9 text-sm font-semibold rounded-lg border border-gray-200 bg-b text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none">
                            ...
                            @else
                                <x-popover>
                                    <form method="post" action="{{ route('task.unstar', ['userId' => $userId, 'spaceId' => $spaceId, 'taskId' => $task->id]) }}" id="dropdown" class="m-0">
                                        @csrf
                                        @method('PUT')
                                        <button type="submit" id="dropdown" class="flex justify-center items-center size-7 sm:size-9 text-sm font-semibold rounded-lg border border-gray-200 bg-b text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none">
